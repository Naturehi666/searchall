...
func init() {
	for _, r := range guize.RegexList {
		...
		if err != nil {
			panic(err) // 如果无法预编译正则表达式，则直接抛出错误
		}
		regexes = append(regexes, re)
	}
}

func SearchConfigFiles(path string, info os.FileInfo, regexList []string) ([]string, error) {
	...
	for _, line := range bytes.Split(lines, []byte{'\n'}) {
		...
		var matches []string
		for _, regex := range regexes { // 新增代码
	...
	if len(matchedContents) > 0 {
		...
		for _, line := range lines {
			...
			lastTwoRegexes := regexList[len(regexList)-4:]

			for _, line := range strings.Split(line, "\n") {
				for _, regex := range lastTwoRegexes {
...
func Searchall(path string) {
	...
	go func() {
		...
		err := filepath.Walk(path, func(path string, info os.FileInfo, err error) error {
			...
			res, err := SearchConfigFiles(path, info, guize.RegexList)
			if err != nil {
				errChan <- err
				return nil
			}
